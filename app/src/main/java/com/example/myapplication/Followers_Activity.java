...
public class Followers_Activity extends AppCompatActivity {
    ...
    // 팔로워 어레이 리스트
    ArrayList<User_DB> Follower_Array = new ArrayList<>();
    ...
    //리사이클러뷰 선언
    RecyclerView Follower_Recycle;
    RecyclerView.LayoutManager Follower_LayoutManager;
    RecyclerView.Adapter Follower_Adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // ?
        Follower_Recycle.setHasFixedSize(true);

        // 레이아웃 매니저 설정
        Follower_LayoutManager = new LinearLayoutManager(getApplicationContext(), LinearLayoutManager.VERTICAL, false);

        // 레이아웃 매니저 set
        Follower_Recycle.setLayoutManager(Follower_LayoutManager);

        // 어뎁터 생성
        Follower_Adapter = new Following_Adapter(Follower_Array, new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                if(obj != null){
                    ...
                    intent.putExtra("Profile_Id", ((Following_Adapter)Follower_Adapter).getData(position).getUser_id());
                    startActivity(intent);
                }

            }
        }, item_db_array);
        Follower_Recycle.setAdapter(Follower_Adapter);
    ...
    public void Follower_Check(){
        // 로그인 유저 팔로워 어레이 만큼 반복
        for(int i = 0; i < User_Db_ArrayList.get(Login_User_Position).getUser_Follower().size(); i++){
            // 전체 유저 수 만큼 어레이 반복
            for(int j = 0; j < User_Db_ArrayList.size(); j++){
                // 팔로워 어레이 유저 아이디 값과 전체 유저 어레이 유저 아이디 값이 같다면
                if(User_Db_ArrayList.get(Login_User_Position).getUser_Follower().get(i).equals(User_Db_ArrayList.get(j).getUser_id())){
                    // 해당 유저 팔로워 어뎁터 어레이에 추가
                    Follower_Array.add(User_Db_ArrayList.get(j));
                    break;
                }
            }
        }
